...
class Sandbox(ABC):
    ...
    普通类可以定义抽象方法，但这种做法没有实际效果。普通类不会强制其子类实现抽象方法，除非它继承自 ABC 类。
    """
...
def _calls_ancestor(program: str, function_to_evolve: str) -> bool:
    ...
    在代码演化过程中，我们可能会生成多个版本的函数，例如 function_to_evolve_v1, function_to_evolve_v2 等。
    这个函数可以帮助我们检测新生成的代码是否在调用旧版本的函数，以确保代码的依赖关系和调用链条是正确的。
    """
...
class Evaluator:
    ...
    def __init__(
            ...
            inputs: Sequence[Any],  # dataset的dict,inputs['OR3']是一个dataset
                                    # RZ: I guess this refers to the evaluate instance
            timeout_seconds: int = 30,
            sandbox_class: Type[Sandbox] = Sandbox  # ipynb里定义的Sandbox，继承了上面定义的Sandbox，默认为上面的Sandbox
    ):
    ...
    def analyse(
        ...
        # print(type(new_function),program)
        # new_function是function_to_evolve的Function  <class 'implementation.code_manipulation.Function'>
        # program 是str 和输入的self._template相同 是去掉了装饰器的所有原始代码段
        scores_per_test = {}  # key是数据集名，value是输出的accuracy什么的
        ...
        for current_input in self._inputs:
            ...
            # self._inputs是数据集的dict，current_input是当前使用的数据集的key (名字str)
