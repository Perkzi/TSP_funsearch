...
class Function:
    ...
    dataclass自动为类生成一些常用的特殊方法，如 __init__、__repr__、__eq__ 等。
    使用 @dataclasses.dataclass 可以减少样板代码
    """
...
class ProgramVisitor(ast.NodeVisitor):
    ...
    这个类的目的是解析代码并收集生成 Program 所需的所有信息。注释中提到，函数装饰器不会被存储。

    """

    def __init__(self, sourcecode: str):
        """
        _codelines：将输入的代码字符串按行分割并存储为列表。

        _preface：用于存储前导代码片段（即在函数定义之前的代码）。

        _functions：用于存储解析到的函数信息的列表。

        _current_function：用于跟踪当前正在解析的函数名称。
        """
    ...
    def visit_FunctionDef(self,  # pylint: disable=invalid-name
        ...
        if node.col_offset == 0:  # We only care about first level functions.只处理顶层函数定义
            ...
            # 如果是第一个函数定义，检查是否有装饰器
            # 如果有装饰器，找到装饰器的最小行号并保留其上面的代码作为 preface。
            # 如果没有装饰器，保留函数定义之前的代码作为 preface。
            if not self._functions:
            ...
            # 提取函数的结束行号 function_end_line 和函数体的起始行号 body_start_line。
            #
            # 如果存在文档字符串（docstring），提取并设置 docstring
    ...
    def return_program(self) -> Program:
        """
        这个方法返回一个 Program 对象，包含前导代码片段（preface）和所有解析到的函数信息（functions）
        """
