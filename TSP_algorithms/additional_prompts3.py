...
'''
这种提示解决了之前的base_prompt容易修改priority函数输入输出逻辑的问题，同时显式提示了TSP problem的解法的潜在问题
'''
tsp_specific_prompt = (
...
step_by_step_prompts = [
    (
        ...
        "Analyze the current TSP problem's approach for calculating priority of one candidate city."
        "Outline a structured, step-by-step optimization logic. "
...
multi_path_analysis_prompts = [
    ...
    (
        "To solve TSP problem, you have to travel like a circle to visit all cities and finally go back to the start city."
        "Analyze the current TSP problem's approach for calculating priority of one candidate city."
        "Considering multiple solution paths that use different heuristics "
...
heuristic_meta_prompts = [
    ...
    (
        "To solve TSP problem, you have to travel like a circle to visit all cities and finally go back to the start city."
        "Analyze the current TSP problem's approach for calculating priority of one candidate city."
        "This approach should combines heuristic methods with metaheuristic optimization techniques."
...
multi_phase_prompts = [
    ...
    (
        ...
        "Break down the solution into distinct phases (e.g., early stage, middle stage, final stage). "
        "For example, if most cities are unvisited, current phase is in the early stage."
