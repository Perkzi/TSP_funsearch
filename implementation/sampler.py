    ...
    def sample(self, **kwargs):
        ...
        while True:
            ...
            if self._max_sample_nums and self.__class__._global_samples_nums >= self._max_sample_nums:
                break
            try:
                # 返回某个island 的 整个program和最新的版本号，里面最新版本的Function还没写内容
                prompt = self._database.get_prompt()
                reset_time = time.time()
                samples = self._llm.draw_samples(prompt.code) # 通过同样的prompt得到多个sample
                sample_time = (time.time() - reset_time) / self._samples_per_prompt
                # This loop can be executed in parallel on remote evaluator machines.
                for sample in samples:
                    self._global_sample_nums_plus_one()  # RZ: add _global_sample_nums
                    cur_global_sample_nums = self._get_global_sample_nums()
                    # 随机选一个evaluator去analyse，结果存入database的对应的一个island的一个cluster？
