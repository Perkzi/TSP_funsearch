...
def main(
    ...
    Args:
        ...
        inputs       : dataset的dict,inputs['OR3']是一个dataset
                        ，the data instances for the problem (see 'bin_packing_utils.py').
        config       : config file.
        max_sample_nums: the maximum samples nums from LLM. 'None' refers to no stop.
    """
    function_to_evolve, function_to_run = _extract_function_names(specification)
    template = code_manipulation.text_to_program(specification)
    print(template.preface,template.functions,type(template))
    """
    [Function(name='get_valid_bin_indices', args='item: float, bins: np.ndarray', 
    body='    return np.nonzero((bins - item) >= 0)[0]', return_type='np.ndarray', 
    docstring='Returns indices of bins in which item can fit.', 
    score=None, global_sample_nums=None, sample_time=None, evaluate_time=None), ...
    长度 21 4   <class 'implementation.code_manipulation.Program'>
    preface都为空？
    preface[0]:  
    functions[0]: def get_valid_bin_indices(item: float, bins: np.ndarray) -> np.ndarray:
    ...
    return priorities
    """
    # 先得到第一个版本的accuracy，存入database的所有island的一个cluster
